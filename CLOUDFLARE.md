...
Переносим **только макет `red_draft3_1_1`** (как основной сайт в корне `/`) и
`frontend/admin` (управление контентом) — раздаёт Pages нативно (CDN, бесплатно).
Остальные версии (`client/`, `v2..v5`, прочие `red_draft*`) на Cloudflare не едут.
Маршруты страниц из Express не портируются — их заменяет статик-раздача Pages +
`_routes.json` (Functions только на `/api/*`).
